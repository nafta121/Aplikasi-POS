...
// ==========================================
// 1. DATA TYPES (INTERNAL UN-EXPORTED TO PREVENT NEXT.JS PAGE TYPE ERRORS)
// ==========================================
interface InventoryItem {
  ...
  category: string;
  unit: string;
  stockToko: number; // Toko Utama (Display & Kasir)
  stockGudang01: number; // Gudang 01 (Semen/Pasir/Besi)
  minStockToko: number;
...
const INITIAL_INVENTORY: InventoryItem[] = [
  {
    ...
    unit: 'Sak',
    stockToko: 6, // < 10 -> Highlight Peringatan
    stockGudang01: 240,
    minStockToko: 15,
  ...
  },
  {
    ...
    category: 'Material Dasar',
    unit: 'Sak',
    stockToko: 4, // < 10 -> Highlight Peringatan
    stockGudang01: 180,
    minStockToko: 15,
  ...
  },
  {
    ...
    category: 'Besi & Baja',
    unit: 'Batang',
    stockToko: 8, // < 10 -> Highlight Peringatan
    stockGudang01: 450,
    minStockToko: 20,
  ...
  },
  {
    ...
    category: 'Pipa & Sanitari',
    unit: 'Batang',
    stockToko: 7, // < 10 -> Highlight Peringatan
    stockGudang01: 120,
    minStockToko: 15,
    rackLocationToko: 'Lorong B Rak Pipa Vertikal',
  ...
  },
  {
    ...
    category: 'Material Dasar',
    unit: 'Truk',
    stockToko: 2, // < 10 -> Highlight Peringatan (satuan truk display)
    stockGudang01: 45,
    minStockToko: 5,
  ...
  },
  {
    ...
    unit: 'Dus',
    stockToko: 5, // < 10 -> Highlight Peringatan
    stockGudang01: 210,
    minStockToko: 12,
...
export default function AdminInventoryPage() {
  ...
  // 4. STATE TAB SURAT JALAN / DELIVERY ORDERS
  ...
  const handleOpenTransferModal = (item: InventoryItem) => {
    ...
    // Default mutasi: jika toko menipis, ambil dari Gudang 01 kirim ke Toko Utama
    setSourceLocation('gudang01');
    setTargetLocation('toko');
    setTransferAmount('20');
    setTransferMemo(`Restock display ${item.name.split(' ')[0]}`);
  ...
  const handleExecuteTransfer = (e: React.FormEvent) => {
    ...
    // Validasi ketersediaan stok di lokasi asal
    ...
    if (amount > availableStock) {
      alert(
        `Stok di ${sourceLocation === 'toko' ? 'Toko Utama' : 'Gudang 01'} tidak mencukupi! Hanya tersedia ${availableStock} ${selectedItemForTransfer.unit}.`
      );
      return;
    }
    ...
    showToast(
      `Berhasil memindahkan ${amount} ${selectedItemForTransfer.unit} "${selectedItemForTransfer.name}" dari ${sourceName} ke ${targetName}.`
    );
  ...
  // Filter Inventory
  const categories = ['Semua', 'Material Dasar', 'Besi & Baja', 'Pipa & Sanitari', 'Cat & Kimia', 'Perkakas & Alat', 'Keramik & Lantai'];
  ...
  const lowStockCountToko = inventoryList.filter((i) => i.stockToko < 10).length;
  const totalGlobalItems = inventoryList.reduce((acc, curr) => acc + curr.stockToko + curr.stockGudang01, 0);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* ==================================================== */}
      {toastMessage && (
        <div className="fixed top-20 right-8 z-50 flex items-center gap-3 p-4 bg-slate-900 dark:bg-slate-800 text-white rounded-2xl shadow-2xl border border-emerald-500/40 animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">
                Inventaris & Logistik Gudang
              </h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Pusat kontrol multi-gudang, mutasi stok material bangunan, dan pelacakan surat jalan armada toko.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                placeholder="Cari SKU atau nama semen, besi, pipa, cat..."
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 px-1 text-xs text-slate-500">
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1.5">
                ...
                <strong className="text-slate-700 dark:text-slate-300">Merah/Kuning:</strong> Stok Toko Utama &lt; 10 (Perlu Mutasi Segera)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block" />
                <strong className="text-slate-700 dark:text-slate-300">Hijau:</strong> Stok Toko Utama Aman
              </span>
            </div>
            <div className="text-slate-400">
              Total Akumulasi Fisik: <strong className="font-mono text-slate-700 dark:text-slate-300">{totalGlobalItems.toLocaleString('id-ID')} unit</strong>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="text-[11px] uppercase tracking-wider bg-slate-100/80 dark:bg-slate-800/80 text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800 font-bold">
                    ...
                    <th className="py-3.5 px-4 text-center font-bold">Gudang 01 (Semen/Pasir)</th>
                  ...
                  {filteredInventory.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-slate-400">
                        Tidak ada material bangunan yang cocok dengan filter.
                      </td>
                    </tr>
                  ) : (
                    filteredInventory.map((item) => {
                      const globalStock = item.stockToko + item.stockGudang01;
                      const isStockTokoCritical = item.stockToko < 10;
                        ...
                        >
                          ...
                          <td className="py-4 px-4">
                            ...
                            <div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-400">
                              ...
                              <span>•</span>
                              <span>Satuan: <strong className="text-slate-700 dark:text-slate-300">{item.unit}</strong></span>
                            </div>
                          </td>
                          ...
                          {/* 4. KOLOM TOKO UTAMA (HIGHLIGHT PERINGATAN KUNING/MERAH JIKA < 10) */}
                          ...
                          <td className="py-4 px-4 text-center whitespace-nowrap font-mono">
                            <div className="inline-flex flex-col items-center">
                              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 font-extrabold text-xs">
                                ...
                                <span>{item.stockGudang01} {item.unit}</span>
                              </span>
                              <span className="text-[10px] text-slate-400 mt-0.5">
                                {item.rackLocationGudang.split(' - ')[1] || 'Gudang Logistik'}
                              </span>
                            </div>
                          </td>

                          {/* 6. KOLOM ACTION (TOMBOL MUTASI) */}
                          <td className="py-4 px-4 text-center whitespace-nowrap">
                            <button
                              id={`btn-mutasi-${item.sku}`}
                              type="button"
                              onClick={() => handleOpenTransferModal(item)}
                              className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all shadow-xs active:scale-95 ${
                                isStockTokoCritical
      ...
      {activeTab === 'surat-jalan' && (
        <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="space-y-3">
                {ordersMenunggu.length === 0 ? (
                  ...
                  ordersMenunggu.map((order) => (
                    ...
                    >
                      {/* Card Header: ID Transaksi & Waktu */}
              ...
              <div className="space-y-3">
                {ordersSiapKirim.length === 0 ? (
                  ...
                  ordersSiapKirim.map((order) => (
                    ...
                    >
                      {/* Card Header: DO Number & Status Badge */}
                      <div className="flex items-center justify-between">
              ...
              <div className="space-y-3">
                {ordersDalamPerjalanan.length === 0 ? (
                  ...
                  ordersDalamPerjalanan.map((order) => (
                    ...
                    >
                      {/* Card Header: DO Number & Live Badge */}
                      <div className="flex items-center justify-between">
          ...
          {/* Section Selesai Terkirim (Collapsed / Historical View) */}
      ...
      {/* ==================================================== */}
      {/* MODAL 1: INTER-WAREHOUSE TRANSFER (MUTASI STOK)     */}
      {/* ==================================================== */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs">
                  ...
                  <div className={`font-mono font-bold text-sm ${selectedItemForTransfer.stockToko < 10 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-900 dark:text-white'}`}>
                    {selectedItemForTransfer.stockToko} {selectedItemForTransfer.unit}
                    {selectedItemForTransfer.stockToko < 10 && ' (Kritis)'}
                  </div>
                </div>
                ...
                <div className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs">
                  ...
                  <div className="font-mono font-bold text-sm text-blue-600 dark:text-blue-400">
                    {selectedItemForTransfer.stockGudang01} {selectedItemForTransfer.unit}
                  </div>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleExecuteTransfer} className="space-y-4 text-xs">
              {/* a. Dropdown Dari Lokasi & Ke Lokasi */}
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      // Otomatis ubah target bila bertabrakan
              ...
              {/* b. Input Jumlah Pindah */}
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  <label className="font-bold text-slate-700 dark:text-slate-300">
                    Jumlah Pindah ({selectedItemForTransfer.unit}):
                  </label>
                ...
                <div className="flex gap-1.5 pt-1">
                  {[10, 20, 50, 100].map((num) => (
                    ...
                    >
                      +{num} {selectedItemForTransfer.unit}
                    </button>
                  ))}
                  <button
                    type="button"
      ...
      {/* ==================================================== */}
      {/* MODAL 2: PREVIEW SURAT JALAN (DELIVERY ORDER FORMAL) */}
      {/* ==================================================== */}
